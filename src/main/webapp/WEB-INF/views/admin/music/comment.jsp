		...
		<div class="layui-row">
		 ...
		 <div class="layui-row">
	    	...
	    	<div id="main-body" class="layui-body">
				...
				<table id="table-comment-list"></table>
	    		<!-- 主体内容 -->
	    		<div class="toolbar">
	    			<div>
...
<script type="text/html" id="comment-detail"></script>
<script type="text/javascript">
	layui.use(['element','layer','table'], function() {
		var element=layui.element;
		var layer=layui.layer;
		var table = layui.table;
		$("#comment-detail").load("${root}/js/module/comment-detail.art");
		$("#table-comment-list").bootstrapTable({
			//height:500,
			striped:true,
			...
			detailFormatter:function(index,row){
			    var repliedArr=$.parseJSON(row.replied);
			    console.log(repliedArr)
				if($.isEmptyObject(repliedArr)){
					return '没有回复内容哦';
				}
				var data={};
				data.items=repliedArr;
				return template('comment-detail',data);
			},
			columns:[
				{checkbox:true,align:'center',width:5},
				{field:'commentId',title:'ID',align:'center',width:5},
		        {field: 'userId', title: '用户ID',align:'center',width:5},
				{field:'nickname',title:'用户昵称',align:'center',width:10},
				{field:'userPhoto',title:'用户图像',align:'center',width:10,formatter:function(value,row,index){
					return '<img height="43px;" src="'+value+'"/>';
				}},
				{field:'content',title:'评论内容',align:'center',width:10},
				{field:'song.songName',title:'歌曲名称',align:'center',width:10,formatter: function(value,row,index){
					return '<a href="${root}/song/'+row.song.songId+'" target="_blank">'+value+'</a>';
				}},
				{field:'song.title',title:'歌曲标题',align:'center',width:10},
				//{field: 'replied', title: '评论回复',align:'center',width:15},
		        {field: 'isHot', title: '热门评论',align:'center',width:5,formatter:function(value,row,index){
		            return value?'是':'否';
		        }},
		        {field: 'praiseCount', title: '被赞数',align:'center',width:5},
		        {field: 'commentTime', title: '评论时间',align:'center',width:10,formatter:function(value,row,index){
		        	return _time(new Date(value),"yyyy-MM-dd hh:ss");
		        }}
			],
			method:'get',
			url:'${root}/api/song/comment',
		...
		$(".table-refresh").click(function(){
			$('#table-comment-list').bootstrapTable('refresh',{silent:true,query:{songName:''}});
		});
		//切换视图
		$(".table-change-view").click(function(){
			$('#table-comment-list').bootstrapTable('toggleView');
		});
		$(".table-del-row").click(function(){
			var selRows=$('#table-comment-list').bootstrapTable('getSelections');
			console.log(selRows);
			var ids=new Array();
		    ...
		    $.ajaxhelper('${root}/api/singer',"post",{ids:ids},function(res){
				...
				$('#table-comment-list').bootstrapTable('remove',{'field':'comment',values:ids});
		    });
		});
	});
</script>
